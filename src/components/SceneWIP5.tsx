...
const ThreeScene = () => {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      if (missileRef.current) {
        ...
        missileRef.current.position.x += direction.x * MISSILE_SPEED;
        missileRef.current.position.y += direction.y * MISSILE_SPEED;
  ...
  const fireMissile = useCallback(() => {
    ...
    const missileGeometry = new THREE.PlaneGeometry(0.2, 0.2);
